import { User, Student, SchoolClass, Subject, ScheduleEntry, SchoolYear, Period, DayEvent, StudentNote } from './types';
...
export async function getEventForClassAndDate(classId: string, date: string): Promise<DayEvent | null> {
  return request<DayEvent | null>(`/events/check?classId=${classId}&date=${date}`);
}
