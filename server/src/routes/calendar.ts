...
// Get all calendar events (list view)
router.get('/', async (_req, res) => {
  try {
    const rows = await query<CalRow>(
      `SELECT id, title, color, event_date, end_date, start_time::text, end_time::text, recurrence, note
       FROM calendar_events ORDER BY event_date, start_time`
    );
...
// Get calendar events for a specific date (recurrence-aware)
...
router.post('/', adminMiddleware, async (req, res) => {
  try {
    const { title, color, date, endDate, startTime, endTime, recurrence, note } = req.body;
    ...
    const row = await queryOne<CalRow>(
      `INSERT INTO calendar_events (title, color, event_date, end_date, start_time, end_time, recurrence, note)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING id, title, color, event_date, end_date, start_time::text, end_time::text, recurrence, note`,
      [title, color || '#3B82F6', date, endDate || null, startTime, endTime, recurrence || 'none', note || null]
    );
  ...
  try {
    ...
    const row = await queryOne<CalRow>(
      `UPDATE calendar_events SET
       ...
       WHERE id = $9
       RETURNING id, title, color, event_date, end_date, start_time::text, end_time::text, recurrence, note`,
      [title || null, color || null, date || null, endDate ?? null, startTime || null, endTime || null, recurrence || null, note ?? null, id]
    );
